...
const EditShowtimes = ({ id, setShowtimesData, selectedDate }: any) => {
  ...
  const onFinish = async (formData: any) => {
    ...
    const formattedData = {
      ...
      selected_date: formatTimeField(formData.selected_date, "YYYY-MM-DD"),
    };
    ...
    mutate(formattedData, {
      ...
      onError: (error: any) => {
        let errorMessage = "Có lỗi xảy ra"; // Giá trị mặc định
        try {
          const parsedError = JSON.parse(error.request.responseText);
          errorMessage = parsedError.message || errorMessage; // Lấy message từ object
        } catch (e) {
          console.error("Lỗi khi parse responseText:", e);
        }
        messageApi.error(errorMessage); // Truyền chuỗi vào messageApi.error
      },
    });
  };
  ...
  const { mutate } = useMutation({
    mutationFn: async (formData) => {
      const response = await axios.put(UPDATE_ONE_SHOWTIMES(id), formData);
  ...
  useEffect(() => {
    if (open && detailShowtimes) {
      form.setFieldsValue({
        ...
        selected_date: selectedDate ? dayjs(selectedDate, "YYYY-MM-DD") : null,
  ...
  //       (item: any) => item.dates === dayjs(selectedDate).format("YYYY-MM-DD")
  //     );
      ...
      >
        ...
        >
          <Form.Item
            ...
            label="Phòng chiếu"
            name="room_id"
            rules={[{ required: true, message: "Thêm phòng chiếu" }]}
          >
          ...
          >
            <Select placeholder="Chọn phim" onChange={handleFilmChange}>
          ...
          >
            <TimePicker
              ...
              disabledTime={() => {
                ...
                return {
                  disabledHours: () =>
                    [...Array(24).keys()].filter((h) => h < startTime.hour()),
          ...
          >
            <Select placeholder="Trạng thái">
              ...
              <Select.Option value="now_showing">Đang chiếu</Select.Option>
              <Select.Option value="referenced">Đã chiếu</Select.Option>
            </Select>
          </Form.Item>
          <Form.Item
            style={{ display: "none" }}
