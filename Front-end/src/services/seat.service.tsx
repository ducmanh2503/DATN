...
const ENDPOINTS = {
  ...
  GET_SEATS_FOR_BOOKING: (roomId: number) =>
    `${BASE_URL}/seats/booking/${roomId}`,
  ...
  DELETE_ALL_SEATS_IN_ROOM: (roomId: number) =>
    `${BASE_URL}/seats/room/${roomId}/delete-all`,
};
...
export const getSeatsForBooking = async (
  roomId: number
): Promise<SeatingMatrix> => {
  try {
    const response = await axios.get<SeatingMatrix>(
      ENDPOINTS.GET_SEATS_FOR_BOOKING(roomId)
    );
    return response.data;
  } catch (error) {
    throw handleApiError(error);
