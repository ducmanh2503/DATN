...
const EditFilm = ({ id }: any) => {
  ...
  const handleFinish = (formData: any) => {
    ...
    // Append các giá trị thông thường
    dataToSend.append("title", formData.title);
    dataToSend.append("trailer", formData.trailer);
    dataToSend.append("movie_status", formData.movie_status);
    dataToSend.append(
      "release_date",
      dayjs(formData.release_date).format("YYYY-MM-DD")
    );
    dataToSend.append("running_time", formData.running_time);
    dataToSend.append("rated", formData.rated);
    dataToSend.append("language", formData.language);
    dataToSend.append("description", formData.description);

    // Chuyển đổi diễn viên từ chuỗi sang mảng nếu cần
    if (typeof formData.actors === "string") {
      const actorsArray = formData.actors
        .split(",")
        .map((actor: string) => actor.trim());
      actorsArray.forEach((actor) => {
        dataToSend.append("name_actors[]", actor);
      });
    }

    // Chuyển đổi thể loại từ chuỗi sang mảng nếu cần
    if (typeof formData.genres === "string") {
      const genresArray = formData.genres
        .split(",")
        .map((genre: string) => genre.trim());
      genresArray.forEach((genre) => {
        dataToSend.append("name_genres[]", genre);
      });
    }
    ...
    // Nếu có file mới thì append poster
    if (selectedFile) {
                ...
                <Form.Item
                  ...
                  rules={[
                    ...
                    },
                    {
                      type: "string",
                      min: 6,
                      message: "Diễn viên phải có ít nhất 6 ký tự",
                    },
                  ]}
                >
