...
const SeatForm: React.FC<SeatFormProps> = ({ onSubmit, onDelete, roomId, seatTypes, seat, isEditing = false, existingSeats }) => {
    ...
    const calculateMaxColumn = (row: string) => {
        ...
        const columns = Object.values(existingSeats[row]).map(seat => parseInt(seat.seatCode.match(/\d+$/)?.[0] || '0', 10));
    ...
    const onFinish = async (values: any) => {
        ...
        try {
            ...
            if (!isEditing && currentSeatCount + numberOfSeats > MAX_SEATS_PER_ROW) {
                message.error(`Hàng ${currentRow} chỉ cho phép tối đa ${MAX_SEATS_PER_ROW} ghế. Hiện tại đã có ${currentSeatCount} ghế.`);
                setLoading(false);
                return;
            }
            ...
            if (isEditing && seat) {
                ...
                const newSeats: SeatCreateRequest[] = Array.from({ length: numberOfSeats }, (_, index) => ({
                    room_id: roomId!,
                    row: values.row,
            ...
            message.error('Lỗi khi xử lý ghế: ' + (error as Error).message || 'Có lỗi xảy ra');
        } finally {
            setLoading(false);
        }
    };
    ...
    const confirmDelete = async () => {
        if (seat && onDelete) {
            ...
            try {
                ...
                message.error('Lỗi khi xóa ghế: ' + (error as Error).message || 'Có lỗi xảy ra');
            } finally {
                setLoading(false);
            }
        }
    ...
    return (
        <Form form={form} onFinish={onFinish} layout="vertical">
            <Spin spinning={loading}>
                ...
                >
                    <Select onChange={handleRowChange} placeholder="Chọn hàng" disabled={isEditing}>
                ...
                >
                    <Input disabled={!isEditing && getCurrentSeatCount(form.getFieldValue('row') || '') > 0} />
                </Form.Item>
                <Form.Item
                    name="seat_type_id"
                    label="Loại Ghế"
                            ...
                            { required: true, message: 'Vui lòng nhập số lượng ghế!' },
                            {
                                validator: (_, value) => {
                                    if (!value || parseInt(value) <= 0) {
                                        return Promise.reject(new Error('Số lượng ghế phải lớn hơn 0!'));
                                    }
                                    const currentRow = form.getFieldValue('row');
                                    const currentSeatCount = getCurrentSeatCount(currentRow);
                                    if (currentSeatCount + parseInt(value) > MAX_SEATS_PER_ROW) {
                                        return Promise.reject(
                                            new Error(`Hàng ${currentRow} chỉ cho phép tối đa ${MAX_SEATS_PER_ROW} ghế. Hiện tại đã có ${currentSeatCount} ghế.`)
                                        );
                                    }
                ...
                <Form.Item>
                    <div style={{ display: 'flex', gap: '8px' }}>
                        <Button
                            type="primary"
                            htmlType="submit"
                            style={{ background: 'var(--backgroud-product)', color: 'var(--word-color)' }}
                            disabled={loading}
                        >
                            {loading ? 'Đang xử lý...' : isEditing ? 'Cập nhật Ghế' : 'Thêm Ghế'}
                        </Button>
                        {isEditing && onDelete && (
                            <Button
