...
const checkStatisticsAccess = () => {
    ...
    if (userRole !== "admin") {
        console.log("[Dashboard Service] User không phải admin, không có quyền truy cập thống kê");
        return false;
    }
    return true;
};
...
export const useDashboard = () => {
    const { data, isLoading, error } = useQuery({
        ...
        queryFn: async () => {
            try {
                ...
                if (!hasAccess) {
                    ...
                    console.log("[Dashboard Service] Trả về dữ liệu mặc định cho user không có quyền");
                    return {
                        total_revenue: 0,
                        total_orders: 0,
                        total_users: 0,
        ...
        onError: (error: any) => {
            ...
            toast.error("Không thể tải dữ liệu thống kê: " + (error.message || "Lỗi không xác định"), {
                position: "top-right",
                autoClose: 3000,
            });
        }
...
export const useExcelExport = () => {
    const mutation = useMutation({
        ...
        onError: (error: any) => {
            ...
            toast.error("Không thể xuất dữ liệu Excel: " + (error.message || "Lỗi không xác định"), {
                position: "top-right",
                autoClose: 3000,
            });
        }
