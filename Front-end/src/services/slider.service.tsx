...
import { Slider, SliderDetailResponse, SliderFormData, SliderResponse } from "../types/slider.type";
...
class SliderService {
  ...
  async getActiveSliders(): Promise<Slider[]> {
    try {
      const response = await axiosInstance.get<SliderResponse>(`${API_ENDPOINT}/active`);
      return response.data.data;
    } catch (error) {
  ...
  async getSlider(id: number): Promise<Slider | null> {
    try {
      const response = await axiosInstance.get<SliderDetailResponse>(`${API_ENDPOINT}/${id}`);
      return response.data.data;
    } catch (error) {
  ...
  async createSlider(formData: SliderFormData): Promise<any> {
    try {
      ...
      if (formData.image instanceof File) {
      ...
      } else if (typeof formData.image === 'object' && formData.image !== null) {
        ...
        try {
          ...
          if (formData.image.originFileObj && formData.image.originFileObj instanceof File) {
            // @ts-ignore
          ...
          throw new Error("Định dạng file không hợp lệ, vui lòng chọn lại hình ảnh");
        }
      } else {
