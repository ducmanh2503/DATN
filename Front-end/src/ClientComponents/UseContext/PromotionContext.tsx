...
export const PromotionProvider = ({
  ...
  const [quantityPromotion, setQuantityPromotion] = useState<number | null>(0);
  const [totalPriceVoucher, setTotalPriceVoucher] = useState<number>(() => {
    const storedTotalPriceVoucher = sessionStorage.getItem("totalPriceVoucher");
    return storedTotalPriceVoucher ? JSON.parse(storedTotalPriceVoucher) : 0;
  });
