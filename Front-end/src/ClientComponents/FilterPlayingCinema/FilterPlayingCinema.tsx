...
const FilterPlayingCinema = ({ onFilterChange, onFilteredDataChange }: FilterPlayingCinemaProps) => {
  ...
  const fetchFilteredMovies = async () => {
    ...
    try {
      ...
      if (activeFilters.releaseYear > 0 || activeFilters.language !== "Tất cả") {
        const params: any = {};
        if (activeFilters.releaseYear > 0) params.release_year = activeFilters.releaseYear;
        if (activeFilters.language !== "Tất cả") params.language = activeFilters.language;
        
        console.log("Gọi API filter với params:", params);
        try {
          const { data } = await axios.get("http://localhost:8000/api/filter", { params });
          ...
          if (activeFilters.language !== "Tất cả") {
            ...
            if (filteredMovies.length > 0) {
              console.log("Các thuộc tính của phim:", Object.keys(filteredMovies[0]));
              console.log("Ngôn ngữ của phim:", filteredMovies[0].language, filteredMovies[0].original_language);
            }
            ...
            filteredMovies = filteredMovies.filter((movie: any) => {
              ...
              return (
                ...
                (movie.original_language && movie.original_language === activeFilters.language)
              );
            });
          }
        }
        ...
        console.log("Gọi API movies-index và lọc theo thể loại:", activeFilters.genre);
        const { data } = await axios.get("http://localhost:8000/api/movies-index");
