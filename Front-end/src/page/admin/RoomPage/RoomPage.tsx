...
const RoomPage: React.FC = () => {
    ...
    const handleUpdateRoom = async (
        ...
        try {
            ...
            message.error(
                error.error || error.message || "Lỗi khi cập nhật phòng"
            );
        } finally {
            setLoading(false);
        }
                ...
                return (
                    <div style={{ display: "flex", gap: "8px" }}>
                        ...
                        {!isDeleted ? (
                            <Button
                                onClick={() =>
                                    showDeleteConfirm([String(room.id)])
                                }
                                style={{
                                    background: "var(--normal-rank)",
                                    color: "var(--word-color)",
                ...
                {rows.map((row) => {
                    ...
                    const rowSeats = Object.values(seats[row] || {})
                        ...
                        .sort((a, b) => {
                            ...
                            return (
                                getColumnNumber(a.seatCode) -
                                getColumnNumber(b.seatCode)
                            );
                        });
                    ...
                    >
                        ...
                        {selectedRoom &&
                            getTotalSeatsCount() >= selectedRoom.capacity && (
                                ...
                                >
                                    Đã đạt giới hạn sức chứa của phòng. Không
                                    thể thêm ghế mới.
                                </div>
                            )}
