...
const SeatInfo = () => {
  ...
  useEffect(() => {
    const newTotalSeatsPrice = typeSeats.reduce((sum: number, seat: any) => {
            ...
            <div className={clsx(styles.seatInfo)}>
              <div className={clsx(styles.seatCount)}>
                <span className={clsx(styles.number)}>
                  {seats.quantitySeats === 0 ? "" : `${seats.quantitySeats} x `}
                </span>
                <span className={clsx(styles.seatType)}>Ghế {seats.type}</span>
              </div>
