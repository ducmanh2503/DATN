...
const ArticleDetail: React.FC = () => {
    ...
    useEffect(() => {
        const fetchArticleDetail = async () => {
            try {
                setLoading(true);
                const response = await axios.get(`http://localhost:8000/api/articles/${id}/client`);
                console.log('Dữ liệu bài viết:', response.data);
                setArticle(response.data);

                const relatedResponse = await axios.get('http://localhost:8000/api/articles-client');
                ...
                <div className="article-detail-container">
                    ...
                    {relatedArticles.length > 0 && (
                        <div className="related-articles">
                            <h2 className="related-title">Bài viết liên quan ({relatedArticles.length})</h2>
