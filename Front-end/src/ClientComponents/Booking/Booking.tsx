...
const Booking = () => {
  ...
  useEffect(() => {
    const loadSeatsAndRoom = async () => {
      ...
      try {
        setLoading(true);
        // Tải ma trận ghế
        const seatsData = await seatService.getSeatsByRoom(
          parseInt(roomId, 10)
        );
        ...
        const errorMessage =
          (error as { error?: string }).error || "Lỗi khi tải dữ liệu";
  ...
  const handleBookTickets = async () => {
    ...
    try {
      ...
      const errorMessage =
        (error as { error?: string }).error || "Lỗi khi đặt vé";
      message.error(errorMessage);
    }
  };

  const renderSeatMatrix = () => {
    return Object.keys(seatingMatrix)
      .sort()
      .map((row) => {
        const rowSeats = Object.values(seatingMatrix[row] || {}).sort(
          (a, b) => {
            const getColumnNumber = (code: string) =>
