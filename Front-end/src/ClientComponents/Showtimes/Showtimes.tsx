                    ...
                    {showtimes && showtimes.length > 0 ? (
                        showtimes.map((showtime) => (
                            <div key={showtime.id} className="showtime-card">
                                <p>Ngày: {new Date(showtime.date).toLocaleDateString("vi-VN")}</p>
                                <p>Giờ bắt đầu: {new Date(showtime.start_time).toLocaleTimeString("vi-VN")}</p>
                                <p>Giờ kết thúc: {new Date(showtime.end_time).toLocaleTimeString("vi-VN")}</p>
                                <p>Phòng: {showtime.room_id}</p>
                                <p>Giá vé: {showtime.price} VND</p>
                                <button
                                    className="book-btn"
                                    onClick={() => handleSelectShowtime(showtime.id, showtime.room_id)}
                                >
                                    Chọn suất và đặt vé
                                </button>
                            </div>
