...
export const CombosProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const [totalComboPrice, setTotalComboPrice] = useState<string | null>(
        () => {
            const storedTotalComboPrice =
                sessionStorage.getItem("totalComboPrice");
            return storedTotalComboPrice
                ? JSON.parse(storedTotalComboPrice)
                : null;
        }
    ...
    useEffect(() => {
        ...
        sessionStorage.setItem(
            "totalComboPrice",
            JSON.stringify(totalComboPrice)
        );
