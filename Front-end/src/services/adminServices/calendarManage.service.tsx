...
export const useCalendarManage = () => {
    const { data, isLoading, isError } = useQuery({
        ...
        queryFn: async () => {
            try {
                console.log("[CalendarManage] Đang gọi API lấy danh sách lịch chiếu với axiosInstance");
...
export const useDeleteCalendar = (messageApi: any) => {
    ...
    const { mutate } = useMutation({
        ...
        onError: (error: any) => {
            messageApi.error(error.response?.data?.message || error.message || "Có lỗi xảy ra khi xóa lịch chiếu!");
        },
    });
...
export const useCreateCalendar = (
    ...
    const { mutate, isPending } = useMutation({
        ...
        onError: (error: any) => {
            messageApi.error(error.response?.data?.message || error.message || "Có lỗi xảy ra khi tạo lịch chiếu!");
        },
    });
...
export const useUpdateCalendar = (
    ...
    const { mutate, isPending } = useMutation({
        ...
        onError: (error: any) => {
            messageApi.error(error.response?.data?.message || error.message || "Có lỗi xảy ra khi cập nhật lịch chiếu!");
        },
    });
    ...
    const { data, isLoading, isError } = useQuery({
        ...
        queryFn: async () => {
            try {
                ...
                console.error(`Lỗi khi kiểm tra lịch chiếu cho phim ID ${movieId}:`, error);
                return false;
            }
        },
        enabled: !!movieId,
