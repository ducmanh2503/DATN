...
import { Layout, Breadcrumb, Button, Table, Modal, message, Spin, ColumnsType } from 'antd';
import roomService from '../../../services/room.service';
import { Room, RoomCreateRequest, RoomUpdateRequest, RoomTableDataSource } from '../../../types/room.types';
...
const RoomPage: React.FC = () => {
                ...
                return (
                    <>
                        ...
                        <Button
                            ...
                            style={{ background: 'var(--normal-rank)', color: 'var(--word-color)', marginRight: 8 }}
    ...
    const paginationConfig = {
        ...
        showTotal: (total: number, range: [number, number]) => `Hiển thị ${range[0]}-${range[1]} trong ${total} phòng`,
    };

    const showDeleteConfirm = (ids: string[]) => {
        Modal.confirm({
            title: 'Xác nhận xóa phòng',
            content: 'Bạn có chắc muốn xóa các phòng đã chọn? Hành động này sẽ xóa mềm (có thể khôi phục).',
            okText: 'Xóa',
            cancelText: 'Hủy',
            onOk: () => handleDeleteRooms(ids),
            okButtonProps: { style: { background: 'var(--normal-rank)', color: 'var(--word-color)', border: 'none' } },
    ...
    const rowSelection = {
        ...
        getCheckboxProps: (record: RoomTableDataSource) => ({
            disabled: rooms.find(r => String(r.id) === String(record.key))?.deleted_at !== null,
        }),
    };
                    ...
                    <div className="room-page-header">
                        ...
                        <div>
                            ...
                            <Button
                                ...
                                style={{ background: '#ff4d4f', color: 'var(--word-color)', border: 'none' }}
