...
export const FilmsProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const [showtimesEndTime, setShowtimesEndTime] = useState<string | null>(
        () => {
            const storedShowtimesEndTime =
                sessionStorage.getItem("showtimesEndTime");
            return storedShowtimesEndTime
                ? JSON.parse(storedShowtimesEndTime)
                : null;
        }
    ...
    const [roomNameShowtimes, setRoomNameShowtimes] = useState<string>(() => {
        const storedRoomNameShowtimes =
            sessionStorage.getItem("roomNameShowtimes");
        return storedRoomNameShowtimes
            ? JSON.parse(storedRoomNameShowtimes)
            : "";
    }); // name room booking
    const [roomTypeShowtimes, setRoomTypeShowtimes] = useState<string>(() => {
        const storedRoomTypeShowtimes =
            sessionStorage.getItem("roomTypeShowtimes");
        return storedRoomTypeShowtimes
            ? JSON.parse(storedRoomTypeShowtimes)
            : "";
    }); // type room booking
