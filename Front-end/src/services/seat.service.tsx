        ...
        Object.keys(processedData).forEach((row) => {
            Object.keys(processedData[row]).forEach((col) => {
                ...
                if (seat.status && typeof seat.status === "object") {
                    ...
                    seat.status = String(
                        statusObj.seat_status || "available"
                    ) as "available" | "booked";
                }
...
export const holdSeat = async (
    ...
    try {
        const response = await axios.post(ENDPOINTS.HOLD_SEAT, {
            seat: seatId,
        });
        return response.data;
    } catch (error) {
...
export const releaseSeat = async (
    ...
    try {
        const response = await axios.post(ENDPOINTS.RELEASE_SEAT, {
            seat: seatId,
        });
        return response.data;
    } catch (error) {
...
export const getSeatTypes = async (): Promise<SeatType[]> => {
    try {
        ...
        try {
            const response = await axios.get<SeatType[]>(
                `${BASE_URL}/seat-types`
            );
