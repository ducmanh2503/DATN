...
import { SearchMovie, SearchRequest, SearchResponse } from "../types/search.types";
 ...
 */
export const searchMovies = async (params: SearchRequest): Promise<SearchMovie[]> => {
  try {
    ...
    return uniqueMovies.filter(movie => {
      ...
      // Tìm theo diễn viên
      if (movie.actors && movie.actors.some(actor => 
      ...
      // Tìm theo đạo diễn
      if (movie.directors && movie.directors.name_director.toLowerCase().includes(keyword)) {
        return true;
      }
...
export const getMovieById = async (movieId: number): Promise<SearchMovie> => {
  try {
    const response = await axios.get<{ data: SearchMovie }>(`${API_URL}/movies-details/${movieId}`);
 ...
 */
export const getRelatedMovies = async (movieId: number): Promise<SearchMovie[]> => {
  try {
    const response = await axios.get<{ data: SearchMovie[] }>(`${API_URL}/movies/${movieId}/related`);
    return response.data.data || [];
  } catch (error) {
    console.error(`Error fetching related movies for movie ID ${movieId}:`, error);
    throw error;
  }
};
