...
const ENDPOINTS = {
    ...
    DELETE_ALL_SEATS_IN_ROOM: (roomId: number) => `${BASE_URL}/seats/deleteAll/${roomId}`,
};
        ...
        Object.keys(processedData).forEach(row => {
            Object.keys(processedData[row]).forEach(col => {
                ...
                if (seat.status && typeof seat.status === 'object') {
                    ...
                    seat.status = String(statusObj.seat_status || 'available') as 'available' | 'booked';
                }
...
export const createSeat = async (seatData: SeatCreateRequest): Promise<{message: string, data: Seat}> => {
    try {
        ...
        try {
            ...
            const response = await axios.post(ENDPOINTS.CREATE_SEAT, validatedSeatData, {
            ...
            if (axios.isAxiosError(innerError) && 
                ...
                const fallbackResponse = await axios.post(ENDPOINTS.CREATE_SEAT, dataWithoutStatus, {
                    headers: { 'Content-Type': 'application/json' }
                });
...
export const deleteAllSeatsInRoom = async (roomId: number): Promise<{message: string}> => {
    try {
        ...
        try {
            ...
            const seatsResponse = await axios.get(ENDPOINTS.GET_SEATS_BY_ROOM(roomId));
            ...
            if (seatIds.length > 0) {
                await Promise.all(seatIds.map(id => axios.delete(ENDPOINTS.DELETE_SEAT(id))));
            }
