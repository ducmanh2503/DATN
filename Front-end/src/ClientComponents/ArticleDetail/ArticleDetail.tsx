...
const ArticleDetail: React.FC = () => {
    ...
    useEffect(() => {
        const fetchArticleDetail = async () => {
            try {
                setLoading(true);
                const response = await axios.get(`${URL_IMAGE}/api/articles/${id}/client`);
                setArticle(response.data);

                const relatedResponse = await axios.get(`${URL_IMAGE}/api/articles-client`);
                const allArticles = relatedResponse.data;

                const related = allArticles
                    .filter((a: Article) => a.id !== response.data.id && a.category === response.data.category);
        ...
        const fetchHotMovies = async () => {
            try {
                console.log('Fetching hot movies from:', `${URL_IMAGE}/api/movies-for-client?status=now_showing`);
                const response = await axios.get(`${URL_IMAGE}/api/movies-for-client`, {
                    params: { status: 'now_showing' },
                });
                console.log('Hot movies API response (full):', JSON.stringify(response.data, null, 2));
                ...
                if (Array.isArray(response.data)) {
                    ...
                    console.error('Invalid movies response format, expected array:', response.data);
                ...
                const formattedMovies = movies.map((movie: any) => ({
                    ...
                    created_at: movie.created_at || movie.release_date || new Date().toISOString(),
                        ...
                        <div className="left-column">
                            <div className="article-detail">
                                ...
                                <div className="article-content">
                                    {article.body || (
                                        <p>
                                            Đây là nội dung mẫu. Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        ...
                        <div className="right-column">
                            <div className="hot-movies">
                                ...
                                <div className="hot-movies-list">
                                    {hotMovies.length > 0 ? (
                                        hotMovies.map(movie => (
                                            <div key={movie.id} className="hot-movie-item" onClick={() => showChill(movie.id)}>
