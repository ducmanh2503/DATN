          ...
          {groupedSeats &&
            Object.entries(groupedSeats).map(([row, seats]) => (
              <div key={row} className={styles.seatRow}>
                {/* Hiển thị tên hàng */}
                <div className={clsx(styles.rowLabel)}>{row}</div>
                <div className={clsx(styles.rowSeats)}>
                  {seats.map((seat) => (
                    <div
                      ...
                      className={clsx(
                        styles.seat,
                        seat.type === "Thường" && styles.normalType,
                        seat.type === "VIP" && styles.VIPType,
                        seat.type === "Sweetbox" && styles.SweetboxType,
                        seat.status === "empty" && styles.empty,
                        seat.status === "disabled" && styles.disabled
                      )}
                      onClick={() =>
                        handleSeatClick(
                          seat.row,
